...
const API = {
    ...
    stores: {
        ...
        register: async (storeData) => {
            const { data: authData, error: authErr } = await _supabase.auth.getUser();
            if (authErr) throw authErr;

            const authUserId = authData?.user?.id;
            if (!authUserId) {
                throw new Error('No authenticated session found. Please log in again and retry.');
            }
    ...
    // ── CONTACT MESSAGES ─────────────────────────────
    contactMessages: {
        // Submit a new message — no auth required (public)
        // Note: no .select() here; anon role has INSERT only, not SELECT
