...
// Create store — uses upsert so it's safe to call even if store already exists
...
// Ensure the logged-in user has a store row. If not, create one automatically.
async function ensureStoreExists(user) {
  let store = await fetchStore(user.id);
  if (!store) {
    // Store was never created (e.g. email confirmation was required at signup).
    // Create it now using the email as a fallback name.
    const fallbackName = user.email?.split('@')[0] || 'My Store';
    await createStore({ id: user.id, name: fallbackName }, getAuthToken());
...
// Add a product — ensures store row exists first to avoid foreign key errors
async function addProduct(product) {
  const user = await getCurrentUser();
  if (!user) throw new Error('Not logged in');

  // Guarantee the store record exists (fixes foreign key constraint error)
...
async function loadDashboard(user) {
  try {
    // ensureStoreExists creates the store row if it was never created (e.g. after email signup)
